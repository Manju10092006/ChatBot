...
function FileChip({file, onRemove}) {
  ...
  useEffect(()=>{
    if(isImg){ const r=new FileReader(); r.onload=e=>setSrc(e.target.result); r.readAsDataURL(file); }
  },[file,isImg]);
  return (
    <div style={S.fileChip}>
      {isImg && src
        ? <img src={src} alt="" style={S.fileThumb}/>
...
// ── Single message ────────────────────────────────────────────────────────────
function Message({msg, idx, speakMsg, speaking, speakingIdx}) {
  ...
  return (
    <div style={{...S.msgRow, justifyContent: isUser?"flex-end":"flex-start",
      ...
      {isUser && <div style={S.userAvatar}>M</div>}
    </div>
  );
}
...
export default function App() {
  ...
  const speakMsg = useCallback((text, idx)=>{
    ...
    if(synth.speaking){ synth.cancel(); setSpeaking(false); setSpeakIdx(null);
      if(speakIdx===idx) return; }
    if(!soundOn) return;
  ...
  async function sendMessage(text) {
    ...
    try {
      // Build Gemini API messages (role "assistant" -> "model" for Gemini)
      ...
      const res = await fetch(GEMINI_URL, {
        ...
        body:JSON.stringify({
          ...
          systemInstruction: {
            parts: [{ text: "You are Aura, a refined, warm, and brilliant AI assistant. You speak with elegance and precision. Responses are clear, insightful, and beautifully structured." }],
          },
      ...
      <div style={{...S.card, ...(dragOver?S.cardDrag:{})}}>
        ...
        <header style={S.header}>
          <div style={S.hLeft}>
            ...
            <div>
              <div style={S.hTitle}>Aura <span style={S.hBadge}>AI</span></div>
        ...
        <div style={S.body}>
          {!started ? (
            <div style={S.welcome}>
              ...
              <h1 style={S.wTitle}>Hello, I'm <em>Aura.</em></h1>
        ...
        <div style={S.inputArea}>
          ...
          <div style={S.footNote}>Aura · Powered by Gemini · Your data stays private</div>
        </div>
      </div>
    </div>
  );
